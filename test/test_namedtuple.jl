...
@testset "from_namedtuple" begin
    ...
    nts = [
        "NamedTuple" => (; (k => randn(rng, nchains, ndraws, v...) for (k, v) in pairs(sizes))...),
    ...
    @testset "posterior::$(type)" for (type, nt) in nts
        ...
        test_namedtuple_data(idata1, :posterior, keys(sizes), nchains, ndraws; library = "MyLib", coords = coords, dims = dims)

        idata2 = convert_to_inference_data(nt; dims = dims, coords = coords, library = "MyLib")
        test_namedtuple_data(idata2, :posterior, keys(sizes), nchains, ndraws; library = "MyLib", coords = coords, dims = dims)
    end

    @testset "$(group)" for group in [:posterior_predictive, :sample_stats, :prior, :prior_predictive, :sample_stats_prior]
