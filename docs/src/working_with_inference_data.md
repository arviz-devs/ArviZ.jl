...
!!! info
	  `Dataset`s are [`DimensionalData.AbstractDimStack`](@extref DimensionalData stacks)s and can be used identically.
...
Its [other selectors](@extref DimensionalData selectors) are also supported.
...
We can rename a dimension in a `Dataset` using DimensionalData's [`set`](@extref DimensionalData set) method:
